...
public class RebindMenuSettingsEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		for (int i = 0; i < menuSettings.toggleableControls.Count; i++)
		{
			...
			menuSettings.toggleableControls[i] = GinputSystems.GinputFileIO.SanitiseStringForSaving(menuSettings.toggleableControls[i]);
		...
		for (int i = 0; i < menuSettings.invertableSmartControls.Count; i++)
		{
			...
			menuSettings.invertableSmartControls[i] = GinputSystems.GinputFileIO.SanitiseStringForSaving(menuSettings.invertableSmartControls[i]);
			if (GUILayout.Button("x"))
			{
		...
		for (int i = 0; i < menuSettings.scalables.Count; i++)
		{
			...
			activeScalable.scalableName = GinputSystems.GinputFileIO.SanitiseStringForSaving(activeScalable.scalableName);
			if (GUILayout.Button("x"))
			{
				deleteScalable = true;
			}
			...
			for (int k = 0; k < activeScalable.scalableSmartControls.Count; k++)
			{
				...
				activeScalable.scalableSmartControls[k] = GinputSystems.GinputFileIO.SanitiseStringForSaving(activeScalable.scalableSmartControls[k]);
				if (GUILayout.Button("x"))
				{

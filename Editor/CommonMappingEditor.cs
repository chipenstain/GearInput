	...
	public override void OnInspectorGUI()
	{
		...
		if (currentPanel == 1)
		{
			...
			for (int i = 0; i < padMapping.buttons.Count; i++)
			{
				...
				activeButton.displayName = GinputFileIO.SanitiseStringForSaving(activeButton.displayName);
		...
		if (currentPanel == 2)
		{
			...
			for (int i = 0; i < padMapping.axis.Count; i++)
			{
				...
				if (axisEditFoldouts[i])
				{
					...
					activeAxis.displayName = GinputFileIO.SanitiseStringForSaving(activeAxis.displayName);
					EditorGUILayout.Space();

		...
		if (currentPanel == 0)
		{
			...
			for (int i = 0; i < controlScheme.controls.Count; i++)
			{
				...
				if (EditorGUILayout.BeginFadeGroup(controlFoldoutAnims[i].faded))
				{
					...
					activeControl.name = GinputSystems.GinputFileIO.SanitiseStringForSaving(activeControl.name);
					...
					for (int k = 0; k < activeControl.virtualInputs.Count; k++)
					{
						...
						activeControl.virtualInputs[k] = GinputSystems.GinputFileIO.SanitiseStringForSaving(activeControl.virtualInputs[k]);
						if (GUILayout.Button("x", EditorStyles.miniButton))
						{
		...
		if (currentPanel == 1)
		{
			...
			for (int i = 0; i < controlScheme.smartControls.Count; i++)
			{
				...
				if (EditorGUILayout.BeginFadeGroup(smartControlFoldouts[i].faded))
				{
					...
					activeSmartControl.name = GinputSystems.GinputFileIO.SanitiseStringForSaving(activeSmartControl.name);

...
namespace GinputSystems
{
	public static class CommonGamepadMappings
	{
		...
		{
			//called when gamepads are plugged in or removed, also when Ginput is first called
			...
			object[] commonMappingAssets = Resources.LoadAll("", typeof(CommonMapping));
		...
		public static List<DeviceInput> GetApplicableMaps(CommonGamepadInputs t, string[] connectedGamepads)
		{
			...
			List<DeviceInput> applicableInputs = new();
			...
			for (int i = 0; i < commonMappings.Count; i++)
			{
				...
				for (int k = 0; k < commonMappings[i].buttons.Count; k++)
				{
					if (commonMappings[i].buttons[k].buttonType == t)
					{
						...
						DeviceInput newInput = new(InputDeviceType.GamepadButton)
						{
							gamepadButtonNumber = commonMappings[i].buttons[k].buttonNumber,
							commonMappingType = t,
							displayName = commonMappings[i].buttons[k].displayName,
							allowedSlots = mappingSlots[i].slots.ToArray()
						};
				...
				for (int k = 0; k < commonMappings[i].axis.Count; k++)
				{
					...
					{
						//add this axis input
						DeviceInput newInput = new(InputDeviceType.GamepadAxis)
						{
							gamepadAxisNumber = commonMappings[i].axis[k].axisNumber,
							commonMappingType = t,
							displayName = commonMappings[i].axis[k].displayName,
							invertAxis = commonMappings[i].axis[k].invert,
							clampAxis = commonMappings[i].axis[k].clamp,
							axisButtoncompareVal = commonMappings[i].axis[k].compareVal,
							defaultAxisValue = commonMappings[i].axis[k].defaultVal,
							allowedSlots = mappingSlots[i].slots.ToArray()
						};

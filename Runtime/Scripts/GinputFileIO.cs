...
{
	public static class GinputFileIO
	{
		...
		private static string LoadString(string schemeName)
		{
		...
		//loading stuff
		private static string[] joysticks;
		...
		public static Control[] LoadFromString(Control[] schemeToReplace, string loadString)
		{
			...
			for (int l = 0; l < loadLines.Length; l++)
			{
				...
				{
					if (int.TryParse(thisLine[1], out int savedControlCount))
					{
						for (int c = 0; c < savedControlCount; c++)
						{
							...
							for (int i = 0; i < savedInputCount; i++)
							{
								...
								thisLine = loadLines[l].Split(seperator[0]);
								//build the input
								DeviceInput currentInput = new((InputDeviceType)Enum.Parse(typeof(InputDeviceType), thisLine[0]))
								{
									displayName = thisLine[1],
									isCustom = thisLine[2] == "True",
									deviceName = thisLine[3]
								};
				...
				{
					if (int.TryParse(thisLine[1], out int savedSmartControlCount))
					{
						for (int c = 0; c < savedSmartControlCount; c++)
						{
							...
							{
								Ginput.SetInverted(currentSmartControl, thisLine[i] == "True", (InputDeviceSlot)i);
							}
							...
							for (int i = 0; i < thisLine.Length; i++)
							{
								Ginput.SetScale(currentSmartControl, float.Parse(thisLine[i]), (InputDeviceSlot)i);
							}
						}
					}
					else
				...
				{
					Ginput.mouseSensitivity = float.Parse(thisLine[1]);
				}
			}
			return controls.ToArray();
		}

		private static void SaveControlsToFile(string schemeName, string saveString)
		{
		...
		private static void SaveControlsToPlayerPrefs(string schemeName, string saveString)
		{
		...
		private static string GenerateSaveString(Control[] controls)
		{
			string saveStr = "";
			...
			saveStr += "smartcontrols" + seperator + Ginput.smartControls.Length.ToString() + "\n";
			for (int s = 0; s < Ginput.smartControls.Length; s++)
			{
				saveStr += Ginput.smartControls[s].name + "\n";
				//inversion settings for each slot
				for (int i = 0; i < Ginput.totalPossibleDeviceSlots; i++)
				{
					saveStr += Ginput.smartControls[s].inversion[i].ToString();
					if (i < Ginput.totalPossibleDeviceSlots - 1) saveStr += seperator;
				}
				saveStr += "\n";
				...
				for (int i = 0; i < Ginput.totalPossibleDeviceSlots; i++)
				{
					saveStr += Ginput.smartControls[s].scales[i].ToString();
					if (i < Ginput.totalPossibleDeviceSlots - 1) saveStr += seperator;
				}
				saveStr += "\n";
			}
			...
			saveStr += "mouseSensitivity" + seperator + Ginput.mouseSensitivity.ToString();
			return saveStr;
		}

...
namespace GinputSystems.XR
{
	public class GinputXR
	{
		private List<GinputXRNode> nodes = new();
		private List<XRNodeState> xrNodeStates = new();
		...
		public void Update()
		{
			...
			for (int i = 0; i < xrNodeStates.Count; i++)
			{
				...
				if (nodeIndex == -1)
				{
					...
					nodes.Add(new GinputXRNode(xrNodeStates[i].nodeType));
				}
				else
				{
		...
		public GinputXRNode GetNode(XRNode nodeType)
		{
			for (int i = 0; i < nodes.Count; i++)
			{
			...
			return new GinputXRNode(nodeType);
		}
	}

	public class GinputXRNode
	{
		public XRNode nodeType;

		public bool isTracked = false;

		public List<int> joystickIndeces = new();
		...
		public GinputXRNode(XRNode newNodeType)
		{
		...
		private bool ButtonCheck(string controlname, ButtonAction bAction)
		{
			...
			for (int i = 0; i < joystickIndeces.Count; i++)
			{
				if (bAction == ButtonAction.DOWN && Ginput.GetButtonDown(controlname, (InputDeviceSlot)joystickIndeces[i])) return true;
				if (bAction == ButtonAction.HELD && Ginput.GetButton(controlname, (InputDeviceSlot)joystickIndeces[i])) return true;
				if (bAction == ButtonAction.UP && Ginput.GetButtonUp(controlname, (InputDeviceSlot)joystickIndeces[i])) return true;
				if (bAction == ButtonAction.REPEATING && Ginput.GetButtonDownRepeating(controlname, (InputDeviceSlot)joystickIndeces[i])) return true;
			}
			return false;
		}
		...
		private float AxisCheck(string controlname, bool getRawValue = false)
		{
			...
			for (int i = 0; i < joystickIndeces.Count; i++)
			{
				if (!getRawValue)
				{
					v = Ginput.GetAxis(controlname, (InputDeviceSlot)joystickIndeces[i]);
				}
				else
				{
					v = Ginput.GetAxisRaw(controlname, (InputDeviceSlot)joystickIndeces[i]);
				}
		...
		private List<QuickVRButtonBinding> quickVRButtons = new();
		void AddBinding(string name, int buttonIndex)
		{
			QuickVRButtonBinding b = new()
			{
				controlName = name,

...
namespace GinputSystems
{
	public class SmartControl
	{
		...
		public void Init()
		{
			...
			rawValues = new float[Ginput.totalPossibleDeviceSlots];
			controlValues = new float[Ginput.totalPossibleDeviceSlots];
			valuePrefersDeltaUse = new bool[Ginput.totalPossibleDeviceSlots];
		...
		public void Update()
		{
			...
			{
				rawValues[slot] = Ginput.GetAxis(positiveControl, (InputDeviceSlot)slot) - Ginput.GetAxis(negativeControl, (InputDeviceSlot)slot);

				if (inversion[slot]) rawValues[slot] *= -1f;

				valuePrefersDeltaUse[slot] = true;
				if (!Ginput.PrefersDeltaUse(positiveControl, (InputDeviceSlot)slot) || !Ginput.PrefersDeltaUse(negativeControl, (InputDeviceSlot)slot))
				{
		...
		{
			if (bAction == ButtonAction.DOWN && Ginput.GetButtonDown(positiveControl, slot)) return true;
			if (bAction == ButtonAction.DOWN && Ginput.GetButtonDown(negativeControl, slot)) return true;
			if (bAction == ButtonAction.HELD && Ginput.GetButton(positiveControl, slot)) return true;
			if (bAction == ButtonAction.HELD && Ginput.GetButton(negativeControl, slot)) return true;
			if (bAction == ButtonAction.UP && Ginput.GetButtonUp(positiveControl, slot)) return true;
			if (bAction == ButtonAction.UP && Ginput.GetButtonUp(negativeControl, slot)) return true;
			return false;
		}
	}
}

...
namespace GinputSystems
{
	public class Control
	{
		...
		public List<CommonGamepadInputs> commonMappings = new();
		...
		//called no more than once a frame from Ginput.GinputUpdate
		bool checkslot = false;
		public void Update()
		{
			if (null == controlStates)
			{
				controlStates = new ControlState[Ginput.totalPossibleDeviceSlots];
			...
			{
				checkslot = false;
				if (i <= Ginput.connectedGamepads) checkslot = true; // slot represents a connected pad
		...
		float v;
		bool wasHeld;
		private void UpdateControlState(int i, InputDeviceSlot slot)
		{
			controlStates[i].value = 0f;
			controlStates[i].valuePrefersDeltaUse = true;
			controlStates[i].axisAsButtonHeld = false;
			...
			if (controlStates[i].held)
			{
				...
				if (controlStates[i].holdTime > Ginput.buttonRepeatWait && controlStates[i].repeatTime < 0f)
				{
					controlStates[i].repeatTime = Ginput.buttonRepeat;
		...
		private void UpdateAnyControlState()
		{
			...
			if (controlStates[0].held)
			{
				...
				if (controlStates[0].holdTime > Ginput.buttonRepeatWait && controlStates[0].repeatTime < 0f)
				{
					controlStates[0].repeatTime = Ginput.buttonRepeat;
		...
		{
			DeviceInput input = new(InputDeviceType.Keyboard)
			{
		...
		{
			Ginput.CheckGamepads();

			if (isNewBinding) commonMappings.Add(gamepadButtonOrAxis);
			List<DeviceInput> applicableMapInputs = CommonGamepadMappings.GetApplicableMaps(gamepadButtonOrAxis, Ginput.gamepads);
		...
		{
			DeviceInput input = new(InputDeviceType.Mouse)
			{
				mouseInputType = mouseInputType,
				commonMappingType = CommonGamepadInputs.NOBUTTON
			};
			inputs.Add(input);
		}
		...
		{
			DeviceInput input = new(InputDeviceType.Virtual)
			{
				virtualInputID = virtualInputID,
				commonMappingType = CommonGamepadInputs.NOBUTTON
			};
			inputs.Add(input);
			...
			for (int i = 0; i < inputs.Count; i++)
			{
				if (inputs[i].isCustom)
				{
					if (inputs[i].inputType == InputDeviceType.GamepadAxis || inputs[i].inputType == InputDeviceType.GamepadButton)
					{
						...
						List<int> allowedSlots = new();
						for (int g = 0; g < Ginput.connectedGamepads; g++)
						{
							if (Ginput.gamepads[g] == inputs[i].deviceName.ToUpper())
							{
								allowedSlots.Add(g);
							}
						}
	...
	class ControlState
	{
		...
		//for Ginput.ButtonPressRepeat() checks

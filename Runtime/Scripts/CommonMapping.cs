...
{
	[CreateAssetMenu(fileName = "New Common Mapping", menuName = "Ginput/Common Mapping", order = 1)]
	public class CommonMapping : ScriptableObject
	{
		public List<string> names = new();//names of gamepads that this mapping can apply to
		public List<string> partialNames = new();//if an exact name match isn't found for a pad, use this mapping if it has a partial match with this
		...
		public List<GamepadButtonInput> buttons = new();
		public List<GamepadAxisInput> axis = new();

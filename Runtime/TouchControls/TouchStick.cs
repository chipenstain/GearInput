...
{
	[AddComponentMenu("Ginput/Touch/TouchStick", 0)]
	public class TouchStick : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if (claimedTouch != -1)
			{
				...
				if (GinputTouchManager.TouchExists(claimedTouch)) stillExists = true;
			...
			if (claimedTouch == -1)
			{
				...
				{
					if (claimedTouch == -1 && Input.touches[i].phase == TouchPhase.Began && !GinputTouchManager.IsClaimed(Input.touches[i].fingerId))
					{
						//this touch press is free, but does it touch the stick?
						if (TouchCollision(Input.touches[i].position, out Vector3 hitPoint, !followTouch))
						{
							if (followTouch)
							{
							...
							GinputTouchManager.ClaimTouch(claimedTouch);
				...
				{
					if (TouchCollision(Input.mousePosition, out Vector3 hitPoint, !followTouch))
					{
						if (followTouch)
						{
							transform.position = hitPoint;
			...
			if (claimedTouch != -1)
			{
				...
				for (int i = 0; i < Input.touchCount; i++)
				{
					...
					{
						//this is our touch
						if (TouchCollision(Input.touches[i].position, out Vector3 hitPoint, false))
						{
				...
				{
					if (TouchCollision(Input.mousePosition, out Vector3 hitPoint, false))
					{
						touchPosition = hitPoint;
					}
				}
		...
		private bool TouchCollision(Vector3 touchPos, out Vector3 hitPoint, bool limitRadius)
		{
			Ray ray = Camera.main.ScreenPointToRay(touchPos);

			stickPlane.SetNormalAndPosition(transform.forward, transform.position);
			if (stickPlane.Raycast(ray, out float hitDistance))
			{
				if (limitRadius)
				{
